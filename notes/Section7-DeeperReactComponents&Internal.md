...
Side-Effect: we mean things like store/remove things in the local storage, send http requests, setTimeout etc.  
Watch lecture 91 for example.
...
How to control when `useEffect` to execute? (ie: not every render, maybe just the first one)  
We can add a second argument to `useEffect` which is an array of all the variables used in the function. It will execute only when the variables change(Also the first render). If we pass an empty array, it will only run once.
...
(我的理解:除了第一次 useEffect 不执行以外，在每次 useEffect 之前执行)  
In the case we give an `empty array []` in the `useEffect` call, the clean up function will only be called when the component is removed.
...
What about in functional component?  
...
When to Optimize?  
The props checks does not comes for free. In the case that, when a parent element changes, the child element is guaranteed also change. We dont want to check the props has changed or not. It will always be different than before. It will be unnecessary to implement `shouldCoponentUpdata` for the child element.
...
`setState` does not only take a JS object, it can also works for a function which takes in previousState and current Props and returns a state that is going to be merged.
...
Think about this case, we want to focus on the last person when we show the person list. The normal way to do this is to use JS querySelector to find that HTML element and add focus to it. But this happends on the whole DOM. React provide another way to do this, the `Ref` property on every elements.  
Two way to do this:

-   function approach (old)

This `inputEl` will be a reference to this `input` tag on the actual HTML page. We can store it in the class and use it .
...
-   constructor approach (new, since React 16.3)
...
In react 16.6+, it added another way to consume context.

-   class based component:
    -   we can access context outside the render. add a special function. `static contextType = AuthContext` by this function, react automatically add a context property to the component. and we dont need `AuthContext.Consumer` anymore. Simple use `this.context` at where it's needed.
-   functional based component:
    -   `useContext()` hook, simply `authContext = useContext(AuthContext)` we are able to retrive context using the `authContext`.
